...
const ProductsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // *** CRITICAL CHANGE: Fetch from your backend API ***
        const res = await fetch('http://localhost:5000/api/products'); // Or use environment variable for backend URL
        if (!res.ok) {
          throw new Error(`HTTP error! status: ${res.status}`);
        }
        const data = await res.json();
        // The backend returns { products: [...], total: ... }
        setProducts(data.products);
      } catch (err) {
  ...
  // Analytics data for category distribution
  // This calculation is fine as it runs after 'products' state is updated
  // and 'products' is initialized as an empty array, so reduce won't fail initially.
  const categoryCounts = products.reduce<Record<string, number>>((acc, product) => {
    acc[product.category] = (acc[product.category] || 0) + 1;
    return acc;
  }, {});
